...
window.CHAMPION_APP_VERSION = "19";
...
window.addEventListener("offline", () => {
  window.atualizarStatusFirebase?.(
    ...
    const textosPaginas = {
      ...
      acessos: ["Login e Senhas", "Gerenciamento dos acessos do sistema"]
    };
...
/* =========================================================
   CENTRAL DE ACESSOS — V19
========================================================= */
let accessUsers = [];
let accessManagerLoaded = false;
...
function accessRoleLabel(role) {
  return {
    ...
    professor: "Professor",
    aluno: "Aluno"
  }[role] || role || "Sem perfil";
}

function accessFormatDate(value) {
...
function accessSetLoading(loading, message = "Carregando acessos...") {
  const box = document.getElementById("accessLoading");
  if (!box) return;
  box.hidden = !loading;
  box.innerHTML = loading ? `<span></span> ${accessEscape(message)}` : "";
}

function accessFillProfiles() {
  const role = document.getElementById("accessRole")?.value;
  const select = document.getElementById("accessProfileId");
  if (!select) return;
  ...
  select.disabled = role === "gestor";
  select.innerHTML = role === "gestor"
    ? '<option value="">Não se aplica</option>'
    : '<option value="">Selecione a pessoa</option>' +
      options
        .sort((a, b) => a.name.localeCompare(b.name, "pt-BR"))
        .map(item => `<option value="${accessEscape(item.id)}" data-name="${accessEscape(item.name)}" data-email="${accessEscape(item.email)}">${accessEscape(item.name)}</option>`)
        .join("");
...
function accessApplyLinkedProfile() {
  const select = document.getElementById("accessProfileId");
  ...
  document.getElementById("accessName").value = name;
  if (email) document.getElementById("accessEmail").value = email;
}

function accessResetForm() {
  const form = document.getElementById("accessForm");
  form?.reset();

  document.getElementById("accessUid").value = "";
  document.getElementById("accessRole").value = "aluno";
  document.getElementById("accessStatus").value = "ativo";
  document.getElementById("accessFormTitle").textContent = "Novo acesso";
  document.getElementById("accessSaveButton").textContent = "Criar acesso";
  document.getElementById("accessCancelEdit").hidden = true;
  document.getElementById("accessPassword").required = true;
  document.getElementById("accessPasswordHelp").textContent =
    "Obrigatória ao criar. Na edição, deixe em branco para manter.";

  accessFillProfiles();
}

function accessGeneratePassword() {
  const alphabet = "ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz23456789!@#";
  const values = crypto.getRandomValues(new Uint32Array(10));
  return [...values].map(value => alphabet[value % alphabet.length]).join("");
}

function accessRenderTable() {
  ...
  document.getElementById("accessTotalUsers").textContent = accessUsers.length;

  if (!filtered.length) {
    tbody.innerHTML = '<tr><td colspan="6"><div class="empty-state">Nenhum acesso encontrado.</div></td></tr>';
    return;
  }
    ...
    <tr>
      <td>
        <div class="access-user-cell">
          <span class="access-avatar">${accessEscape((user.displayName || user.email || "?").slice(0, 2).toUpperCase())}</span>
          <div>
            <strong>${accessEscape(user.displayName || "Sem nome")}</strong>
            <small>${accessEscape(user.profileId || "Sem vínculo")}</small>
          </div>
        </div>
      </td>
      <td><span class="access-role access-role-${accessEscape(user.role)}">${accessEscape(accessRoleLabel(user.role))}</span></td>
      <td>${accessEscape(user.email || "-")}</td>
      <td>
        <span class="access-state ${user.disabled ? "blocked" : "active"}">
          ${user.disabled ? "Bloqueado" : "Ativo"}
        </span>
      </td>
      <td>${accessEscape(accessFormatDate(user.lastSignInTime))}</td>
      <td>
        <div class="access-row-actions">
          <button type="button" class="table-action" onclick="accessEditUser('${accessEscape(user.uid)}')">Editar</button>
          <button type="button" class="table-action" onclick="accessResetPassword('${accessEscape(user.uid)}')">Nova senha</button>
          <button type="button" class="table-action danger" onclick="accessDeleteUser('${accessEscape(user.uid)}')">Excluir</button>
        </div>
      </td>
    </tr>
  `).join("");
...
async function accessCall(action, payload = {}) {
  if (typeof window.firebaseGerenciarAcesso !== "function") {
    throw new Error(
      "As Cloud Functions ainda não foram publicadas. Siga o arquivo LEIA-ME-V19."
    );
  }

  return window.firebaseGerenciarAcesso(action, payload);
}

async function accessLoadUsers(force = false) {
  if (accessManagerLoaded && !force) {
    accessRenderTable();
    return;
  }

  accessSetLoading(true);

  try {
    const result = await accessCall("list");
    accessUsers = Array.isArray(result?.users) ? result.users : [];
    accessManagerLoaded = true;
    accessRenderTable();
  } catch (error) {
    console.error("Falha ao listar acessos:", error);
    document.getElementById("accessTableBody").innerHTML =
      `<tr><td colspan="6"><div class="empty-state access-error">${accessEscape(error.message || "Não foi possível carregar os acessos.")}</div></td></tr>`;
  } finally {
    accessSetLoading(false);
  }
}

window.accessEditUser = function(uid) {
  const user = accessUsers.find(item => item.uid === uid);
  if (!user) return;

  document.getElementById("accessUid").value = user.uid;
  document.getElementById("accessRole").value = user.role || "aluno";
  accessFillProfiles();
  document.getElementById("accessProfileId").value = user.profileId || "";
  document.getElementById("accessName").value = user.displayName || "";
  document.getElementById("accessEmail").value = user.email || "";
  document.getElementById("accessPassword").value = "";
  document.getElementById("accessPassword").required = false;
  document.getElementById("accessStatus").value = user.disabled ? "bloqueado" : "ativo";
  document.getElementById("accessFormTitle").textContent = "Editar acesso";
  document.getElementById("accessSaveButton").textContent = "Salvar alterações";
  document.getElementById("accessCancelEdit").hidden = false;
  document.getElementById("accessPasswordHelp").textContent =
    "Deixe em branco para manter a senha atual.";

  document.getElementById("acessos")?.scrollIntoView({ behavior: "smooth" });
};

window.accessResetPassword = async function(uid) {
  const user = accessUsers.find(item => item.uid === uid);
  if (!user) return;

  const password = accessGeneratePassword();

  const confirmed = confirm(
    `Redefinir a senha de ${user.displayName || user.email} para:\n\n${password}\n\nConfirma?`
  );
  if (!confirmed) return;

  try {
    accessSetLoading(true, "Redefinindo senha...");
    await accessCall("update", { uid, password });
    await navigator.clipboard?.writeText(password);
    mostrarAlerta(`Senha redefinida: ${password} (copiada).`);
  } catch (error) {
    mostrarAlerta(error.message || "Não foi possível redefinir a senha.", "error");
  } finally {
    accessSetLoading(false);
  }
};

window.accessDeleteUser = async function(uid) {
  const user = accessUsers.find(item => item.uid === uid);
  if (!user) return;

  const confirmed = confirm(
    `Excluir definitivamente o acesso de ${user.displayName || user.email}?\n\nO cadastro da pessoa não será apagado, apenas a conta de login.`
  );
  if (!confirmed) return;

  try {
    accessSetLoading(true, "Excluindo acesso...");
    await accessCall("delete", { uid });
    mostrarAlerta("Acesso excluído.");
    accessManagerLoaded = false;
    await accessLoadUsers(true);
  } catch (error) {
    mostrarAlerta(error.message || "Não foi possível excluir o acesso.", "error");
  } finally {
    accessSetLoading(false);
  }
};

document.getElementById("accessRole")?.addEventListener("change", () => {
  accessFillProfiles();
  document.getElementById("accessName").value = "";
  document.getElementById("accessEmail").value = "";
});

document.getElementById("accessProfileId")?.addEventListener("change", accessApplyLinkedProfile);
document.getElementById("accessCancelEdit")?.addEventListener("click", accessResetForm);
document.getElementById("accessRefreshButton")?.addEventListener("click", () => accessLoadUsers(true));
document.getElementById("accessSearch")?.addEventListener("input", accessRenderTable);
document.getElementById("accessRoleFilter")?.addEventListener("change", accessRenderTable);

document.getElementById("accessGeneratePassword")?.addEventListener("click", async () => {
  const password = accessGeneratePassword();
  const input = document.getElementById("accessPassword");
  input.type = "text";
  input.value = password;
  await navigator.clipboard?.writeText(password);
  mostrarAlerta("Senha provisória gerada e copiada.");
});

document.getElementById("accessTogglePassword")?.addEventListener("click", () => {
  const input = document.getElementById("accessPassword");
...
document.getElementById("accessForm")?.addEventListener("submit", async event => {
  ...
  const uid = document.getElementById("accessUid").value;
  const role = document.getElementById("accessRole").value;
  const profileId = document.getElementById("accessProfileId").value;
  const displayName = document.getElementById("accessName").value.trim();
  const email = document.getElementById("accessEmail").value.trim().toLowerCase();
  const password = document.getElementById("accessPassword").value;
  const disabled = document.getElementById("accessStatus").value === "bloqueado";
  const button = event.submitter;

  if (role !== "gestor" && !profileId) {
    mostrarAlerta("Selecione a pessoa vinculada ao acesso.", "error");
    return;
  }

  if (!uid && password.length < 6) {
    mostrarAlerta("Informe uma senha com pelo menos 6 caracteres.", "error");
    return;
  }

  try {
    button.disabled = true;
    button.textContent = uid ? "Salvando..." : "Criando...";

    const action = uid ? "update" : "create";
    await accessCall(action, {
      uid,
      role,
      profileId,
      displayName,
    ...
    mostrarAlerta(uid ? "Acesso atualizado." : "Acesso criado.");
    accessResetForm();
    accessManagerLoaded = false;
    await accessLoadUsers(true);
  } catch (error) {
    mostrarAlerta(error.message || "Não foi possível salvar o acesso.", "error");
  } finally {
    button.disabled = false;
    button.textContent = document.getElementById("accessUid").value
      ? "Salvar alterações"
      : "Criar acesso";
  }
});

const accessOriginalMenuClick = document.querySelector('.menu button[data-view="acessos"]');
accessOriginalMenuClick?.addEventListener("click", () => {
  accessFillProfiles();
  accessLoadUsers();
});
...
(function(){
  ...
  window.firebaseCriarUsuario=async({role,profileId,nome,email,cpf,password})=>{
    ...
    const emailContato=String(email||"").trim().toLowerCase();

    const loginEmail=role==="aluno"
      ? emailContato
      : role==="professor"
    ...
    const senhaAcesso=(role==="aluno"||role==="professor")
      ? cpfNumeros.slice(0,6)
      : String(password||"");

    if(role==="aluno" && !loginEmail){
  ...
  window.firebaseMigrarAcessoAluno=async({
    authUid,
    cpf,
    emailNovo,
    nome,
    ...
    if(currentProfile?.role!=="gestor"){
      throw new Error("Somente o gestor pode migrar acessos.");
    }

    const cpfNumeros=String(cpf||"").replace(/\D/g,"");
    const senhaPadrao=cpfNumeros.slice(0,6);
    const emailInterno=cpfEmail(cpfNumeros,"aluno");
    const novoEmail=String(emailNovo||"").trim().toLowerCase();

    const secondaryName="student-migrator-"+Date.now();
    const secondary=firebase.initializeApp(firebaseConfig,secondaryName);
    ...
    try{
      ...
      throw new Error(
        "Não foi possível migrar o acesso antigo automaticamente. " +
        "O aluno pode já ter alterado a senha. Nesse caso, recrie a conta no Firebase Authentication."
      );
    }finally{
      await secondary.delete();
    }
  ...
  window.firebaseLogin=(identifier,password)=>{
    ...
    loginPromise=(async()=>{
      ...
      try{
        ...
        if(!identificador){
          throw new Error("Informe o e-mail ou CPF.");
        }
        ...
        if(loginPorEmail){
          cred=await comTimeout(
            ...
            8000,
            "O Firebase demorou para responder ao login."
          );
        }else{
          ...
          cred=await comTimeout(
            ...
            8000,
            "O Firebase demorou para responder ao login do professor."
          );
        }
        ...
        /*
          Libera a entrada imediatamente após autenticar e carregar o perfil.
          A base continua carregando em segundo plano.
        */
        ready=true;
        window.CHAMPION_FIREBASE_READY=true;
        ...
        status(
          "online",
          `Acesso liberado · sincronizando dados · versão ${window.CHAMPION_APP_VERSION}`
        );

        const carregamento = currentProfile.role==="aluno"
          ? loadStudent(cred.user.uid)
          : loadManagerProfessor();
        ...
        carregamento
          ...
          .catch((erroCarga) => {
            console.error("Falha no carregamento em segundo plano:", erroCarga);
            status(
              "offline",
              "Login realizado. Alguns dados ainda não foram sincronizados."
            );
          });
